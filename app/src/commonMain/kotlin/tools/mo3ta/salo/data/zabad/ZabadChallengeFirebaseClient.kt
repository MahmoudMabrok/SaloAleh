...
import tools.mo3ta.salo.domain.ZabadChallengeDayStats
import tools.mo3ta.salo.domain.ZabadLeaderboardEntry

private const val ROOT_PATH = "zabad_challenge"
...
private const val TOTAL_TODAY_ZABAD_KEY = "totalTodayZabad"

class ZabadChallengeFirebaseClient(
    private val mirror: FirestoreMirror,
    private val analyticsManager: AnalyticsManager,
) {

    private val log = Logger.withTag("ZabadChallengeFirebase")
        ...
        return runCatching {
            Firebase.database.reference(userPath(dateKey, uid)).updateChildren(
                mapOf(
                    ...
                    DATA_KEY to mapOf(
                        ...
                        GOAL_KEY to ZABAD_CHALLENGE_DAILY_GOAL,
                        COMPLETED_KEY to (count >= ZABAD_CHALLENGE_DAILY_GOAL),
            ...
            result.fold(
                onSuccess = {
                    ...
                    mirror.mirrorZabadUserDay(
                        dateKey, uid, count, countryCode, safeNickname,
                        ZABAD_CHALLENGE_DAILY_GOAL, count >= ZABAD_CHALLENGE_DAILY_GOAL, streak,
                    )
                },
                onFailure = { error ->
    ...
    suspend fun fetchDayStats(dateKey: String, uid: String): Result<ZabadChallengeDayStats> {
        ...
        return runCatching {
            ...
            val participantCountSnapshot = dayRef.child(PARTICIPANT_COUNT_KEY)
                .valueEvents
                .first()
            val totalTodayZabadSnapshot = dayRef.child(TOTAL_TODAY_ZABAD_KEY)
                .valueEvents
                .first()

            ZabadChallengeDayStats(
                rank = (rankSnapshot.value as? Number)?.toInt() ?: 0,
                participantCount = (participantCountSnapshot.value as? Number)?.toInt() ?: 0,
                totalTodayZabad = (totalTodayZabadSnapshot.value as? Number)?.toInt() ?: 0,
            )
        }.also { result ->
            result.fold(
                onSuccess = { log.d { "fetchDayStats[$dateKey/$uid] rank=${it.rank} participants=${it.participantCount} totalToday=${it.totalTodayZabad}" } },
                onFailure = { error ->
    ...
    suspend fun fetchLeaderboard(dateKey: String): Result<List<ZabadLeaderboardEntry>> {
        ...
        return runCatching {
            ...
            val entries = parseZabadLeaderboardEntries(rawValue)
...
internal fun parseZabadLeaderboardEntries(value: Any?): List<ZabadLeaderboardEntry> {
    ...
    return indexedValues
        .sortedBy { it.first }
        .mapNotNull { (_, rawEntry) -> rawEntry.toZabadLeaderboardEntry() }
}

private fun Any?.toZabadLeaderboardEntry(): ZabadLeaderboardEntry? {
    val entry = this as? Map<*, *> ?: return null
    val uid = entry[UID_KEY] as? String ?: return null
    return ZabadLeaderboardEntry(
        uid = uid,
