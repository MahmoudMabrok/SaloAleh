...
import tools.mo3ta.salo.domain.AlBaqaraChallengeDayStats
import tools.mo3ta.salo.domain.AlBaqaraLeaderboardEntry

private const val ROOT_PATH = "albaqara_challenge"
...
private const val TOTAL_TODAY_KEY = "totalTodayAlBaqara"

class AlBaqaraChallengeFirebaseClient(
    private val mirror: FirestoreMirror,
    private val analyticsManager: AnalyticsManager,
) {

    private val log = Logger.withTag("AlBaqaraChallengeFirebase")
        ...
        return runCatching {
            Firebase.database.reference(userPath(dateKey, uid)).updateChildren(
                mapOf(
                    ...
                    DATA_KEY to mapOf(
                        ...
                        GOAL_KEY to ALBAQARA_CHALLENGE_DAILY_GOAL,
                        COMPLETED_KEY to (count >= ALBAQARA_CHALLENGE_DAILY_GOAL),
            ...
            result.fold(
                onSuccess = {
                    ...
                    mirror.mirrorAlBaqaraUserDay(
                        dateKey, uid, count, countryCode, safeNickname,
                        ALBAQARA_CHALLENGE_DAILY_GOAL, count >= ALBAQARA_CHALLENGE_DAILY_GOAL,
                    )
                },
                onFailure = { error ->
    ...
    suspend fun fetchDayStats(dateKey: String, uid: String): Result<AlBaqaraChallengeDayStats> {
        ...
        return runCatching {
            ...
            val participantCountSnapshot = dayRef.child(PARTICIPANT_COUNT_KEY)
                .valueEvents
                .first()
            val totalTodaySnapshot = dayRef.child(TOTAL_TODAY_KEY)
                .valueEvents
                .first()

            AlBaqaraChallengeDayStats(
                rank = (rankSnapshot.value as? Number)?.toInt() ?: 0,
                participantCount = (participantCountSnapshot.value as? Number)?.toInt() ?: 0,
                totalTodayAlBaqara = (totalTodaySnapshot.value as? Number)?.toInt() ?: 0,
            )
        }.also { result ->
            result.fold(
                onSuccess = { log.d { "fetchDayStats[$dateKey/$uid] rank=${it.rank} participants=${it.participantCount} totalToday=${it.totalTodayAlBaqara}" } },
                onFailure = { error ->
    ...
    suspend fun fetchLeaderboard(dateKey: String): Result<List<AlBaqaraLeaderboardEntry>> {
        ...
        return runCatching {
            ...
            parseAlBaqaraLeaderboardEntries(snap.value)
        }.also { result ->
            result.fold(
...
internal fun parseAlBaqaraLeaderboardEntries(value: Any?): List<AlBaqaraLeaderboardEntry> {
    ...
    return indexedValues
        .sortedBy { it.first }
        .mapNotNull { (_, rawEntry) -> rawEntry.toAlBaqaraLeaderboardEntry() }
}

private fun Any?.toAlBaqaraLeaderboardEntry(): AlBaqaraLeaderboardEntry? {
    val entry = this as? Map<*, *> ?: return null
    val uid = entry[UID_KEY] as? String ?: return null
    return AlBaqaraLeaderboardEntry(
        uid = uid,
