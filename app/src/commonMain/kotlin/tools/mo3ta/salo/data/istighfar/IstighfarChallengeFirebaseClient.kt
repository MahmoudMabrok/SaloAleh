...
private const val ROOT_PATH = "istighfar_challenge"
...
class IstighfarChallengeFirebaseClient(
            ...
            Firebase.database.reference(userPath(dateKey, uid)).updateChildren(
                mapOf(
                    ...
                    DATA_KEY to mapOf(
                        ...
                        GOAL_KEY to ISTIGHFAR_CHALLENGE_DAILY_GOAL,
                        COMPLETED_KEY to (count >= ISTIGHFAR_CHALLENGE_DAILY_GOAL),
            ...
            result.fold(
                onSuccess = {
                    ...
                    mirror.mirrorIstighfarUserDay(
                        ...
                        ISTIGHFAR_CHALLENGE_DAILY_GOAL, count >= ISTIGHFAR_CHALLENGE_DAILY_GOAL, streak,
                    )
                },
                onFailure = { error ->
    ...
    suspend fun fetchDayStats(dateKey: String, uid: String): Result<IstighfarChallengeDayStats> {
        ...
        return runCatching {
            ...
            val participantCountSnapshot = dayRef.child(PARTICIPANT_COUNT_KEY)
                .valueEvents
                .first()
            val totalTodayIstighfarSnapshot = dayRef.child(TOTAL_TODAY_ISTIGHFAR_KEY)
                .valueEvents
                .first()

            IstighfarChallengeDayStats(
                rank = (rankSnapshot.value as? Number)?.toInt() ?: 0,
                participantCount = (participantCountSnapshot.value as? Number)?.toInt() ?: 0,
                totalTodayIstighfar = (totalTodayIstighfarSnapshot.value as? Number)?.toInt() ?: 0,
            )
        }.also { result ->
            result.fold(
    ...
    suspend fun fetchLeaderboard(dateKey: String): Result<List<IstighfarLeaderboardEntry>> {
        ...
        return runCatching {
            ...
            val entries = parseIstighfarLeaderboardEntries(rawValue)
...
internal fun parseIstighfarLeaderboardEntries(value: Any?): List<IstighfarLeaderboardEntry> {
    ...
    return indexedValues
        .sortedBy { it.first }
        .mapNotNull { (_, rawEntry) -> rawEntry.toIstighfarLeaderboardEntry() }
}

private fun Any?.toIstighfarLeaderboardEntry(): IstighfarLeaderboardEntry? {
    val entry = this as? Map<*, *> ?: return null
    val uid = entry[UID_KEY] as? String ?: return null
    return IstighfarLeaderboardEntry(
        uid = uid,
