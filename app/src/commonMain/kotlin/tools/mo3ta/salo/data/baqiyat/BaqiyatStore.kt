...
/**
 * The phrases of one cycle: the four the "hum of the bees" hadith itself names — tasbih, tahmid,
 * takbir, tahlil. These are what the screen shows and what the hive canvas sends around the Throne.
 */
...
class BaqiyatStore(private val settings: Settings) {
     ...
     * locally-counted cycle (taps + applied manual entries) so the overall total-over-time is
    ...
    private fun publishWeekly(today: LocalDate, total: Int): Int =
        settings.syncWeeklyGoal("baqiyat", today, total)
    ...
    private companion object {
        ...
        const val KEY_LIFETIME = "baqiyat_challenge_lifetime"
    }
}
