package tools.mo3ta.salo.data.kalimat
...
import tools.mo3ta.salo.domain.KalimatChallengeDayStats
import tools.mo3ta.salo.domain.KalimatLeaderboardEntry

private const val ROOT_PATH = "kalimat_challenge"
...
private const val TOTAL_TODAY_KEY = "totalTodayKalimat"

class KalimatChallengeFirebaseClient(
    private val analyticsManager: AnalyticsManager,
) {

    private val log = Logger.withTag("KalimatChallengeFirebase")
            ...
            Firebase.database.reference(userPath(dateKey, uid)).updateChildren(
                mapOf(
                    ...
                    DATA_KEY to mapOf(
                        ...
                        GOAL_KEY to KALIMAT_CHALLENGE_DAILY_GOAL,
                        COMPLETED_KEY to (count >= KALIMAT_CHALLENGE_DAILY_GOAL),
    ...
    suspend fun fetchDayStats(dateKey: String, uid: String): Result<KalimatChallengeDayStats> {
        ...
        return runCatching {
            ...
            KalimatChallengeDayStats(
                rank = (rankSnapshot.value as? Number)?.toInt() ?: 0,
                participantCount = (participantCountSnapshot.value as? Number)?.toInt() ?: 0,
                totalTodayKalimat = (totalTodaySnapshot.value as? Number)?.toInt() ?: 0,
            )
        }.also { result ->
            result.fold(
                onSuccess = { log.d { "fetchDayStats[$dateKey/$uid] rank=${it.rank} participants=${it.participantCount} totalToday=${it.totalTodayKalimat}" } },
                onFailure = { error ->
    ...
    suspend fun fetchLeaderboard(dateKey: String): Result<List<KalimatLeaderboardEntry>> {
        ...
        return runCatching {
            ...
            parseKalimatLeaderboardEntries(snap.value)
        }.also { result ->
            result.fold(
...
internal fun parseKalimatLeaderboardEntries(value: Any?): List<KalimatLeaderboardEntry> {
    ...
    return indexedValues
        .sortedBy { it.first }
        .mapNotNull { (_, rawEntry) -> rawEntry.toKalimatLeaderboardEntry() }
}

private fun Any?.toKalimatLeaderboardEntry(): KalimatLeaderboardEntry? {
    val entry = this as? Map<*, *> ?: return null
    val uid = entry[UID_KEY] as? String ?: return null
    return KalimatLeaderboardEntry(
        uid = uid,
