...
import tools.mo3ta.salo.domain.AlfHasanaChallengeDayStats
import tools.mo3ta.salo.domain.AlfHasanaLeaderboardEntry

private const val ROOT_PATH = "alf_hasana_challenge"
...
private const val TOTAL_TODAY_KEY = "totalTodayAlfHasana"

class AlfHasanaChallengeFirebaseClient(
    private val analyticsManager: AnalyticsManager,
) {

    private val log = Logger.withTag("AlfHasanaChallengeFirebase")
            ...
            Firebase.database.reference(userPath(dateKey, uid)).updateChildren(
                mapOf(
                    ...
                    DATA_KEY to mapOf(
                        ...
                        GOAL_KEY to ALF_HASANA_CHALLENGE_DAILY_GOAL,
                        COMPLETED_KEY to (count >= ALF_HASANA_CHALLENGE_DAILY_GOAL),
    ...
    suspend fun fetchDayStats(dateKey: String, uid: String): Result<AlfHasanaChallengeDayStats> {
        ...
        return runCatching {
            ...
            AlfHasanaChallengeDayStats(
                rank = (rankSnapshot.value as? Number)?.toInt() ?: 0,
                participantCount = (participantCountSnapshot.value as? Number)?.toInt() ?: 0,
                totalTodayAlfHasana = (totalTodaySnapshot.value as? Number)?.toInt() ?: 0,
            )
        }.also { result ->
            result.fold(
                onSuccess = { log.d { "fetchDayStats[$dateKey/$uid] rank=${it.rank} participants=${it.participantCount} totalToday=${it.totalTodayAlfHasana}" } },
                onFailure = { error ->
    ...
    suspend fun fetchLeaderboard(dateKey: String): Result<List<AlfHasanaLeaderboardEntry>> {
        ...
        return runCatching {
            ...
            parseAlfHasanaLeaderboardEntries(snap.value)
        }.also { result ->
            result.fold(
...
internal fun parseAlfHasanaLeaderboardEntries(value: Any?): List<AlfHasanaLeaderboardEntry> {
    ...
    return indexedValues
        .sortedBy { it.first }
        .mapNotNull { (_, rawEntry) -> rawEntry.toAlfHasanaLeaderboardEntry() }
}

private fun Any?.toAlfHasanaLeaderboardEntry(): AlfHasanaLeaderboardEntry? {
    val entry = this as? Map<*, *> ?: return null
    val uid = entry[UID_KEY] as? String ?: return null
    return AlfHasanaLeaderboardEntry(
        uid = uid,
