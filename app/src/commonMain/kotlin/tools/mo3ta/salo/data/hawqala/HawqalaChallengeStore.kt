...
/**
 * Local ledger for the "كنوز الجنة" hawqala challenge — mirrors
 * [tools.mo3ta.salo.data.alfhasana.AlfHasanaChallengeStore].
 * Total to display = remote baseline + unsynced pending taps. Never touches the network.
 */
class HawqalaChallengeStore(private val settings: Settings) {
     ...
     * locally-counted hawqala (taps + applied manual entries) so the overall total-over-time is
    ...
    private companion object {
        ...
        const val KEY_REMOTE = "hawqala_challenge_count"
        const val KEY_PENDING = "hawqala_challenge_pending"
        const val KEY_MANUAL = "hawqala_challenge_manual"
        // Lifetime accumulator — survives day rollover; total counted across all days.
        const val KEY_LIFETIME = "hawqala_challenge_lifetime"
    }
}
