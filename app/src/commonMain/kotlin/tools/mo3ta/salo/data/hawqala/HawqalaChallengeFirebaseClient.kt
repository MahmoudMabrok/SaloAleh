...
import tools.mo3ta.salo.domain.HawqalaChallengeDayStats
import tools.mo3ta.salo.domain.HawqalaLeaderboardEntry

private const val ROOT_PATH = "hawqala_challenge"
...
private const val TOTAL_TODAY_KEY = "totalTodayHawqala"

class HawqalaChallengeFirebaseClient(
    private val analyticsManager: AnalyticsManager,
) {

    private val log = Logger.withTag("HawqalaChallengeFirebase")
            ...
            Firebase.database.reference(userPath(dateKey, uid)).updateChildren(
                mapOf(
                    ...
                    DATA_KEY to mapOf(
                        ...
                        GOAL_KEY to HAWQALA_CHALLENGE_DAILY_GOAL,
                        COMPLETED_KEY to (count >= HAWQALA_CHALLENGE_DAILY_GOAL),
    ...
    suspend fun fetchDayStats(dateKey: String, uid: String): Result<HawqalaChallengeDayStats> {
        ...
        return runCatching {
            ...
            HawqalaChallengeDayStats(
                rank = (rankSnapshot.value as? Number)?.toInt() ?: 0,
                participantCount = (participantCountSnapshot.value as? Number)?.toInt() ?: 0,
                totalTodayHawqala = (totalTodaySnapshot.value as? Number)?.toInt() ?: 0,
            )
        }.also { result ->
            result.fold(
                onSuccess = { log.d { "fetchDayStats[$dateKey/$uid] rank=${it.rank} participants=${it.participantCount} totalToday=${it.totalTodayHawqala}" } },
                onFailure = { error ->
    ...
    suspend fun fetchLeaderboard(dateKey: String): Result<List<HawqalaLeaderboardEntry>> {
        ...
        return runCatching {
            ...
            parseHawqalaLeaderboardEntries(snap.value)
        }.also { result ->
            result.fold(
...
internal fun parseHawqalaLeaderboardEntries(value: Any?): List<HawqalaLeaderboardEntry> {
    ...
    return indexedValues
        .sortedBy { it.first }
        .mapNotNull { (_, rawEntry) -> rawEntry.toHawqalaLeaderboardEntry() }
}

private fun Any?.toHawqalaLeaderboardEntry(): HawqalaLeaderboardEntry? {
    val entry = this as? Map<*, *> ?: return null
    val uid = entry[UID_KEY] as? String ?: return null
    return HawqalaLeaderboardEntry(
        uid = uid,
