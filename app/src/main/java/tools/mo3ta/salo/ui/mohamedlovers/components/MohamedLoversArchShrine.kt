...
internal fun MohamedLoversArchShrine(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.mohamed_lovers_shrine_label),
            style = TextStyle(
        ...
        Text(
            text = stringResource(R.string.mohamed_lovers_shrine_sub),
            style = TextStyle(
...
private fun DrawScope.drawArch(glow: Float) {
    ...
    // Mihrab arch path: rounded top, near-flat bottom
    ...
    // Gold outer border (color brightens with glow)
    val outerColor = lerpColor(
        MohamedLoversPalette.GoldBase,
    ...
    // Inner filigree crosshatch — clip to the inner arch path, then draw diagonals across
    ...
    // Keystone gold dot (pulses with glow)
    val keyCx = w / 2f
    ...
    // Halo when lit (radial glow centered at the arch base)
    if (glow > 0f) {
        drawCircle(
            brush = Brush.radialGradient(
