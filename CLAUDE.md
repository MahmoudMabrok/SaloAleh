...
Continuous daily-activity streak earned by sending salawat. The streak can be frozen for up to 3 Cairo days; 3 consecutive days with no zikr reset it.

- Core files: `data/engagement/RoundStreakStore.kt`, `domain/EngagementModels.kt` (`Achievement.RoundStreakBadge`, `ROUND_STREAK_TARGET = 7`, `STREAK_FREEZE_MAX_DAYS = 3`), `presentation/MohamedLoversViewModel.kt`, `ui/AchievementsScreen.kt`, `ui/components/RoundStreakInfoDialog.kt`, `ui/StreakBadgeAnnouncementDialog.kt`.
- Local persistence via `multiplatform-settings` keys `round_streak_last_active`, `round_streak_count`, `round_streak_badges`, `round_streak_freeze_until`, `round_streak_reset_shown`. The streak is **continuous across rounds** — it is never reset on a round boundary. Missing 1–2 days consumes the freeze window and keeps the streak; **3 consecutive days with no zikr** restart it at 1 (display 0 until the next zikr). Users can freeze remaining days in the window from the streak dialog. The `roundKey` passed to `recordActivity` is used solely to award one repeatable badge per round at 7 consecutive active days. (Legacy installs may still carry an orphaned `round_streak_round_key` key; it is no longer read.)
- Every salawat path (tap, manual sheet, extension sync) records activity and **publishes the current streak to `players/{uid}/roundStreak`** (client, fire-and-forget, only on change) so it shows next to the name on the leaderboard. Hidden when `0`/absent. Bootstrap calls `reconcile` so a 3-day miss writes 0 and publishes the clear.
...
- The daily cron `generate-stats.js` **breaks streaks after 3 consecutive inactive days** — it increments `mohamed_lovers/streakInactivity/{uid}/consecutiveInactiveDays` when `totalCount <= yesterdayTotalScore`, resets it to 0 on an active day, and clears `roundStreak` in `players` and `leaderboard` only once the counter reaches 3. The inactivity node is round-independent so a Friday rollover cannot reset the miss count.
...
- Screen order: header, **hadith transcript + sanad (directly under the header, always whole)**, external-entry button, the scale (takes the space left between transcript and counter), the counter (ring trimmed 220→176dp to make room), tap hint, then the rewards button. The transcript is measured before anything that can flex and carries no `maxLines`, so a short screen shrinks the scale — never the narration.
...
The server-published daily badge is treated as evidence of a **minimum** day count: a badge only lands after the score that earned it reached the server (`publishDailyBadgeIfChanged` runs after the flush), so a local count below the badge's threshold means the device lost today's progress — a reinstall, cleared storage, or a mid-day device switch. Local medal state (`currentDailyBadge` + the milestone celebration) is applied by `noteDailyBadgeProgress` on every scoring path — taps, manual ("record external") entry, and extension sync — so a bulk add that crosses a threshold does not wait for a later tap to show or publish the medal.
