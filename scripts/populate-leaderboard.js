...
async function populateQuranChallengeToday(db) {
  ...
  const usersSnap = await db.ref(`${QURAN_CHALLENGE_ROOT}/${dateKey}/users`).get();
  const users = [];
  ...
  const oldLbSnap = await db.ref(`${QURAN_CHALLENGE_ROOT}/${dateKey}/leaderboard`).get();
  const oldRanks = buildOldRankMap(oldLbSnap);
  ...
  const updates = {
    ...dailyRanking.rankUpdates,
    [`${QURAN_CHALLENGE_ROOT}/${dateKey}/participantCount`]: dailyRanking.participantCount,
    [`${QURAN_CHALLENGE_ROOT}/${dateKey}/totalTodayQuran`]: dailyRanking.totalTodayQuran,
    [`${QURAN_CHALLENGE_ROOT}/${dateKey}/lastRankedAt`]: admin.database.ServerValue.TIMESTAMP,
    [`${QURAN_CHALLENGE_ROOT}/${dateKey}/leaderboard`]: Object.fromEntries(leaderboardEntries),
  };

  await db.ref('/').update(updates);
  console.log(
    `Wrote quran ranks + leaderboard(${leaderboardEntries.length}) for ${dailyRanking.participantCount} participant(s). totalTodayQuran=${dailyRanking.totalTodayQuran}`,
  );

  // Phase 1: mirror to Firestore
  await mirrorQuranChallenge(admin.firestore(), dateKey, {
    rankedUsers: dailyRanking.rankedUsers,
    participantCount: dailyRanking.participantCount,
...
async function populateAlfHasanaChallengeToday(db) {
  ...
  const usersSnap = await db.ref(`${ALF_HASANA_CHALLENGE_ROOT}/${dateKey}/users`).get();
  const users = [];
  ...
  const oldLbSnap = await db.ref(`${ALF_HASANA_CHALLENGE_ROOT}/${dateKey}/leaderboard`).get();
  const oldRanks = buildOldRankMap(oldLbSnap);
  ...
  await attachChallengeMedals(db, ALF_HASANA_CHALLENGE_ROOT, leaderboardEntries);

  const updates = {
    ...dailyRanking.rankUpdates,
    [`${ALF_HASANA_CHALLENGE_ROOT}/${dateKey}/participantCount`]: dailyRanking.participantCount,
    [`${ALF_HASANA_CHALLENGE_ROOT}/${dateKey}/totalTodayAlfHasana`]: dailyRanking.totalTodayAlfHasana,
    [`${ALF_HASANA_CHALLENGE_ROOT}/${dateKey}/lastRankedAt`]: admin.database.ServerValue.TIMESTAMP,
    [`${ALF_HASANA_CHALLENGE_ROOT}/${dateKey}/leaderboard`]: Object.fromEntries(leaderboardEntries),
  };

  await db.ref('/').update(updates);
  console.log(
    `Wrote alf_hasana ranks + leaderboard(${leaderboardEntries.length}) for ${dailyRanking.participantCount} participant(s). totalTodayAlfHasana=${dailyRanking.totalTodayAlfHasana}`,
  );
...
async function populateHawqalaChallengeToday(db) {
  ...
  const usersSnap = await db.ref(`${HAWQALA_CHALLENGE_ROOT}/${dateKey}/users`).get();
  const users = [];
  ...
  const oldLbSnap = await db.ref(`${HAWQALA_CHALLENGE_ROOT}/${dateKey}/leaderboard`).get();
  const oldRanks = buildOldRankMap(oldLbSnap);
  ...
  await attachChallengeMedals(db, HAWQALA_CHALLENGE_ROOT, leaderboardEntries);

  const updates = {
    ...dailyRanking.rankUpdates,
    [`${HAWQALA_CHALLENGE_ROOT}/${dateKey}/participantCount`]: dailyRanking.participantCount,
    [`${HAWQALA_CHALLENGE_ROOT}/${dateKey}/totalTodayHawqala`]: dailyRanking.totalTodayHawqala,
    [`${HAWQALA_CHALLENGE_ROOT}/${dateKey}/lastRankedAt`]: admin.database.ServerValue.TIMESTAMP,
    [`${HAWQALA_CHALLENGE_ROOT}/${dateKey}/leaderboard`]: Object.fromEntries(leaderboardEntries),
  };

  await db.ref('/').update(updates);
  console.log(
    `Wrote hawqala ranks + leaderboard(${leaderboardEntries.length}) for ${dailyRanking.participantCount} participant(s). totalTodayHawqala=${dailyRanking.totalTodayHawqala}`,
  );

  // Top-3 change notifications (drop-out / lost position), like mohamed_lovers.
  const top3Notifs = computeTop3Changes(oldRanks, dailyRanking.rankedUsers);
  await sendTop3ChangeNotifications(db, admin, top3Notifs, CHALLENGE_TOP3_MESSAGES.hawqala, 'hawqala');
}
...
async function populateZabadChallengeToday(db) {
  ...
  const usersSnap = await db.ref(`${ZABAD_CHALLENGE_ROOT}/${dateKey}/users`).get();
  const users = [];
  ...
  const oldLbSnap = await db.ref(`${ZABAD_CHALLENGE_ROOT}/${dateKey}/leaderboard`).get();
  const oldRanks = buildOldRankMap(oldLbSnap);
  ...
  const updates = {
    ...dailyRanking.rankUpdates,
    [`${ZABAD_CHALLENGE_ROOT}/${dateKey}/participantCount`]: dailyRanking.participantCount,
    [`${ZABAD_CHALLENGE_ROOT}/${dateKey}/totalTodayZabad`]: dailyRanking.totalTodayZabad,
    [`${ZABAD_CHALLENGE_ROOT}/${dateKey}/lastRankedAt`]: admin.database.ServerValue.TIMESTAMP,
    [`${ZABAD_CHALLENGE_ROOT}/${dateKey}/leaderboard`]: Object.fromEntries(leaderboardEntries),
  };

  await db.ref('/').update(updates);
  console.log(
    `Wrote zabad ranks + leaderboard(${leaderboardEntries.length}) for ${dailyRanking.participantCount} participant(s). totalTodayZabad=${dailyRanking.totalTodayZabad}`,
  );

  // Phase 1: mirror to Firestore
  await mirrorZabadChallenge(admin.firestore(), dateKey, {
    rankedUsers: dailyRanking.rankedUsers,
    participantCount: dailyRanking.participantCount,
